...
//%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%  DESCRIPTION   %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
/* 
* This class performs all operations to build one connected network
* It connects isolated routes to the network and makes crossing routes intersect at stops to enhance network effect
*/ 
//%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%  DESCRIPTION   %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
public class EvoOpsMerger {
	...
	public static void mergeRoutes(MNetworkPop newPopulation, Network globalNetwork, Double maxConnectingDistance,
		...
		for (MNetwork mn : newPopulation.networkMap.values()) {
			...
			// Check if any two routes cross. If they do, try to make smooth intersection of both in a mutual stop.
			for (MRoute r1 : mn.routeMap.values()) {
				...
				for (MRoute r2 : mn.routeMap.values()) {
					...
					for (List<String> routePair : crossedRoutePairs) {
						// continue if routes have already been paired
...
//					Log.write("01 - Do routes cross [ "+r1.routeID + " x " + r2.routeID + " ] ??");
					// check is routes actually cross
					// use only one way of the route instead of the entire sequence with return
					// check every possible link pair, if the links coincide or cross
...
//							Log.write("crossLink2 = "+ crossLink2.getId().toString() + "=" + link2Id.toString());
							// check if routes cross anyways on a mutual stop or if they cross somewhere along a link and
							// have to be modified so that they intersect on a mutual stop.
							// --> Routes cross without modification if
							// 	a. they share an identical link and there is a stop facility on that link
							//	b. the links touch on two of their nodes and there is a stop facility present at that node location
...
//								Log.write("01a - Yes, Routes do cross BY OVERLAPPING LINK/NODE with a facility [ "+r1.routeID + " x " + r2.routeID + " ]");
							...
							// if the routes cross not on a mutual link, modify so that they do
...
//								Log.write("01a - Yes, Routes do cross [ "+r1.routeID + " x " + r2.routeID + " ]");
								...
								// return true if the routes have successfully been modified for smooth intersection
...
//			Log.write("02 - Initial crossing complete: Crossed route pairs are = "+crossedRoutePairs.toString() + "-------------------------------");
			...
			// check for all routes if they have been part of at least one merger pair.
			// If not, try to connect their endings
			// And connect their endings anyways to improve network effect
...
//			Log.write("crossedRoutePairs = "+crossedRoutePairs);
			for (MRoute mr : mn.routeMap.values()) {
				...
				if (routeIsConnected.equals(false)) {
					...
					// can also connect terminals anyways of already connected routes if possible without big effort
					// --> use smaller distance=400m, and smaller angles=90° only!)
			...
			// this is the case when there is more than one resulting autonomousMetroSubnetworks
			// however, it is unlikely that this happens as the above loops have fiercly attempted to connect all routes
	...
	public static void connectRouteToNetwork(MRoute mr, MNetwork mn, Double maxConnectingDistance, Network globalNetwork,
		...
		for (String routeId : mn.routeMap.keySet()) {
			...
			// make sure route is not trying to be connected to itself
			...
			// as mergers must occur through a mutual stop, all possible stopFacilities along the other route,
			// with which the merger is attempted are listed
...
//						Log.write("Adding possible intersect facility: "+tsf.getName());
		...
		// check if terminals of to be connected route are within reach of another stop facility in the network
		// take the closest connection possible
...
//				Log.write("Distance of closest feasible stop is too high for reasonable connecting. Will therefore not connect unattached route="+mr.routeID);
			...
			// if method has not returned yet by this point, make actual connection by modifying the to be connected route (linking its terminal)
...
//			Log.write("Trying to connect a close facility "+closestFacility.getName());
			// check is the close connection distance was from the start or end terminal
...
//				Log.write(" ... to end terminal facility "+metroLinkAttributes.get(routeEndTerminal.getId()).toNodeStopFacility.getName());
				// it was the end terminal
				// find the shortest connecting path
				...
				if (connectingNodeList.size() == 0) {
					// failed to build a shortest path
...
//					Log.write(" --> No shortest path. Removing from possible facilities "+closestFacility.getName());
				...
				// modify route attributes with new routing
...
//					Log.write(" --> Turning angles not ok. Removing from possible facilities "+closestFacility.getName());
				...
				// Start terminal was closest connection point. Proceed analogously to above procedure.
...
//					Log.write(possibleIntersectFacilities.toString());
	...
	// link nicely two routes by modifying them such that they intersect along a mutual stop
...
//			Log.write("Route1 already has facility on crossing link = "+thisIntersectionLinkFacilityR1.getName());
			...
			// find next link with facility by roaming up the link list of the route
...
//					Log.write("Route1 found close upstream facility = "+r1nextTSFup.getName());
			...
			// do the same for the other direction. CAUTION: REVERSE link list this time as we proceed in the opposite direction!
...
//		Log.write("New linkList = "+r2.linkList.subList(0, r2.linkList.size()/2).toString());
	...
	// this method checks if two links cross
	public static boolean linksDoCross(Link link1, Link link2) throws IOException {
		...
		// make linear functions for links:   [y = m*x + q]
		if (x1A == x1B) {
		...
		// make all possible intersection conditions --> linear functions must intersect between the endings of both links
