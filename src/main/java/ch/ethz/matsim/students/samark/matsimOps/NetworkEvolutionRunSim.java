...
//%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%  DESCRIPTION   %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
/* 
* This class has three essential tasks.
* First, it contains the methods to run the MATSim simulation.
* Secondly, it contains the methods to process the events of the simulation and the resulting agent plans.
* Thirdly, it holds the recall module to recall simulations at a specific generation.
*/
//%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%  DESCRIPTION   %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
import ch.ethz.matsim.students.samark.eventHandlerCounter.MHandlerPassengers;
...
public class NetworkEvolutionRunSim {
	...
	public static void run(String[] args, MNetwork mNetwork, String initialRouteType, 
		...
		// setting simulation path to the specific MNetwork that is simulated
		...
		// sets the options for the initial agent plans as specified in NetworkEvolution
		...
		// use mergedFiles that have been created in createMNetworks
...
//						}
	...
	// runs through the output events of the simulation to assess all metro usage stats
...
//				Log.write("Iteration = "+thisIteration);
				// read and handle events - MHandlerPassengers particularly checks for agents who use the metro
				// the object mPassengerHandler stores all the stats of the EventHandler
...
//				Log.write("Correct mPassengerHandler TOTAL metro distance = "+mPassengerHandler.totalMetroPersonDist);
				...
				// add up the metro usage for the individual metro routes
...
//			}
	...
	// scans the resulting agent plans to assess the entire PT usage
	public static MNetworkPop peoplePlansProcessingM(MNetworkPop networkPopulation, int maxTravelTimeInSec,
		...
		for (MNetwork mNetwork : networkPopulation.networkMap.values()) {
			...
			// all these parameters are necessary to later on calculate the cost benefit analysis
			Double ptUsers = 0.0;
			Double carUsers = 0.0;
			...
			// Average the events output over several iteration (generationsToAverage).
				...
				for (Person person : finalPlansPopulation.getPersons().values()) {
					...
					for (PlanElement element : plan.getPlanElements()) {
						if (element instanceof Leg) {
							...
							// obviously, only if specified in NetworkEvolution
							...
							// go through all legs and sum up travel times by their modes
							if (leg.getMode().contains("car")) {
								carTimeTotal += leg.getTravelTime();
								carPersonDist += leg.getRoute().getDistance();
								isCarTraveler = true;
							}
							// XXX SEE THESIS FOR EXPLANATION OF NUMERICAL FACTORS BELOW
...
//						}
	...
	public static void recallSimulation(MNetworkPop latestPopulation, Map<Id<Link>, CustomMetroLinkAttributes> metroLinkAttributes,
		...
		// remove excess networkMaps if it is too long
		// this happens when a sim is recalled at an earlier gen and has already written later gen's into the file
		// the later gen's are therefore cut off
		...
		// again cut-off it pedigree holds entries after the recall generation
...
//			pedigreeTree.removeAll(pedigreeTree.subList(generationToRecall-1, pedigreeTree.size()));
			List<Map<String, String>> pedigreeTreeTmp = new ArrayList<Map<String, String>>();
			pedigreeTreeTmp.addAll(pedigreeTree.subList(0, generationToRecall-1));
			pedigreeTree = pedigreeTreeTmp;
		}
		else if (pedigreeTree.size() < generationToRecall-1) {
			Map<String,String> defaultMap = new HashMap<String,String>();
			for (int g=1; g<=latestPopulation.networkMap.size(); g++) {
				defaultMap.put("Network"+g, "Network"+g);
			}
			int genDiff = generationToRecall-1-pedigreeTree.size();
			for (int g=0; g<genDiff; g++) {
				pedigreeTree.add(0, defaultMap);
			}
		}
		XMLOps.writeToFile(pedigreeTree, "zurich_1pm/Evolution/Population/HistoryLog/pedigreeTree.xml");			
		
		// recall all MRoutes at the specified generation and build a network from them
