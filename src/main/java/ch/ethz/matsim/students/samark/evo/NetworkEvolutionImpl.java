...
import ch.ethz.matsim.students.samark.dijkstra.DemoDijkstra;
import ch.ethz.matsim.students.samark.dijkstra.DijkstraOwn_I;
import ch.ethz.matsim.students.samark.eventHandlerCounter.PT_StopTrafficCounter;
...
import ch.ethz.matsim.students.samark.od.OD_ProcessorImpl;
...
public class NetworkEvolutionImpl {
	...
	// created networks from scratch and fills them up with routes and transit line services thereon
	public static MNetworkPop createMNetworks(String populationName, int populationSize, int initialRoutesPerNetwork, String initialRouteType, 
		...
		// which should be the same iteration number as the one that will be simulated. this is kind of the steady state before introducing a metro network
		// and will show which links are frequented strongly
		...
		// metroLinkAttributes mark stopFacilities along the link, define if a link was created from an existing railway or new etc.
		...
		if (mergeMetroWithRailway == true) { // merge with outerCity existing metroNetwork
			...
			// Manual inputs here e.g. knowledge from transport planner
		...
		// find all possible stopFacilities, which could be used as end terminals for making the new metro routes
...
//				Sets.newHashSet("pt"), "zurich_1pm/Evolution/Population/BaseInfrastructure/GlobalNetwork.xml");
		...
		for (int N=1; N<=populationSize; N++) {														// Make individual networks one by one in loop
			...
			// new routes are built either
			// - manually
			// - per shortest path between random terminals
			// - or per shortest path between most frequent OD-pairs
			// depending on the options chosen in NetworkEvolution or VCEvolution
			if (useOdPairsForInitialRoutes==false) {
				if (manualODInput) {
					...
					initialMetroRoutes = NetworkEvolutionImpl.createInitialRoutesRandom(metroNetwork, shortestPathStrategy,
							terminalFacilityCandidates, allMetroStops, initialRoutesPerNetwork, zurich_NetworkCenterCoord, metroCityRadius,
							varyInitRouteSize,
							minInitialTerminalDistance, minInitialTerminalRadiusFromCenter, maxInitialTerminalRadiusFromCenter);					
				}
...
//				Log.write(metroNetwork...);
			...
			for(int lineNr=1; lineNr<=nTransitLines; lineNr++) {
				...
				// NetworkRoute: fill MRoute with one of the networkRoutes above
				NetworkRoute metroNetworkRoute = initialMetroRoutes.get(lineNr-1);
				// the routes in an MNetwork are built from MRoute. In addition to the networkRoute itself, the MRoute class contains info such
				// as number of vehicles on each route, departures and any other necessary attributes to fully specify the services and routings (see MRoute class)
				...
				// for easier routes processing, save simple linkSequence in addition to standard networkRoute
			...
			// merge metro with original scenario (network, schedule, vehicles)
			...
			// do this so EVO/SIM loop knows to consider these networks for processing
			networkPopulation.modifiedNetworksInLastEvolution.add(thisNewNetworkName);
			networkPopulation.addNetwork(mNetwork);
		}
		return networkPopulation;
	}

	// this method is used to fill the network with an entire grid of potential metro stops instead of using just the most frequent existing stops
	// --> every original stop in the original scenario is also considered a potential stop for the metro
	// this can be useful if the frequent stops do not cover the map or if reasonable traffic info is not available as may be the case for a virtual city
	public static Map<Id<Link>, CustomLinkAttributes> createEntireMetroGrid(Network network) {
		...
		for (TransitStopFacility tsf : ScenarioUtils.loadScenario(ConfigUtils.loadConfig("zurich_1pm/zurich_config.xml")).getTransitSchedule().getFacilities().values()) {
			if (network.getLinks().containsKey(tsf.getLinkId()) &&  !customLinkMap.containsKey(tsf.getLinkId())){
				// mark links if they access a stop facility in the original network
				// later, when roaming along these links to find accessible stops, these can all be considered
	...
	// the existing railway network is to be connected to the metro network so that the metro vehicles can also switch to the railway network
	public static void connectRailStopsInMetroNetworkToSurroundingNodes(Network metroNetwork,
		...
		Network newConnectionsNetwork = sc.getNetwork();	// this newConnectionsNetwork is only built for visualization of the new connections
		
		for (CustomStop railStop : railStops.values()) {
			List<Id<Node>> alreadyConnectedNodes = new ArrayList<Id<Node>>();
			// for each rail stop, find the corresponding facility and the node lying below that facility
			...
			// check if the node below the railStop is close enough to a node (with a metro facility on top!) in the metro network
			// so that it can be connected by a new link
			for (Node node : metroNetwork.getNodes().values()) {
				...
				if (GeomDistance.betweenNodes(facilityNode, node) < 500.0) {
					...
					// checking, which tsf is on the other node, which has been found in the proximity
	...
	// examine the selected agent plans files to extract key performance indicators to calculate welfare=benefit-cost
	public static CBPII calculateCBAStats(String plansFolder, String outputFile, int populationFactor,
		...
		// Average the events output over several iterations (generationsToAverage).
		// For every generation add its performance divided by its single weight
			...
			for (Person person : finalPlansPopulation.getPersons().values()) {
				...
				for (PlanElement e : selectedPlan.getPlanElements()) {
					if (e instanceof Leg) {
						...
						// obviously, only if specified by user
						...
						// add up travel times according to the mode used for a travel leg
						if (leg.getMode().contains("car")) {
							...
							isPtTraveler = true;
							// XXX CONSULT THESIS FOR EXPLANATION OF MONETARY FACTORS BELOW
...
//			System.out.println("Number of links above threshold is: " + linksAboveThreshold.size());
		...
		// close links (within metro2metroCatchmentArea), that have been selected, can be merged to one link
		// the same is done for links close to an original rail stop with a catchment area of railway2metroCatchmentArea (it does not make sense to
		// have a railway stop and metro stop very close by but not connect the two)
		public static Map<Id<Link>, CustomLinkAttributes> mergeLinksWithinBounds( boolean entireMetroGrid, Map<Id<Link>, CustomLinkAttributes> links_withinRadius, 
			...
			// this is not done if the entireMetroGrid option is chosen as - for that case - we want to make use of all links, no matter how close!
			if (entireMetroGrid) {
				return links_withinRadius;
			}
			
			// SECTION 1: Merge links with railwayStopFacility if their dominant facilities are in vicinity of existing railwayStop & set reference railwayStation (= closest railwayStation)
			...
			//Use the following line if SECTION 1 is not activated:
			//Map<Id<Link>, CustomLinkAttributes> metroRailwayMergedLinks = Clone.customLinkMap(links_withinRadius); // just a copy so that we can play around and remove entries
			Map<Id<Link>, CustomLinkAttributes> metroRailwayMetroMergedLinks = new HashMap<Id<Link>, CustomLinkAttributes>();
			
			do {
				// go through all links (= the selected links after having undergone the railway merger procedure)
				...
				// take note that this link has been added to the new bunch of close metro merged links and must not be considered anymore
				// it will be deleted after the adding procedure
				toBeDeletedLinks.add(thisLink);

				metroRailwayMetroMergedLinks.put(thisLink, metroRailwayMergedLinks.get(thisLink));
				// when adding a new link, check against all other already metro merged links if it is close to one of them and must not as well be added
				for (Id<Link> otherLink : metroRailwayMergedLinks.keySet()) {
					if(thisLink.equals(otherLink)) {
						continue;
					}
					// make sure the new link def. has a dominant stop facility in place, which can be used as location for a new metro stop facililty
...
//						Log.write("This one is ok...");
					...
					// check if it close enough to another link to not be added
				...
				// remove this link from the links to be attempted to add
				for (Id<Link> l : toBeDeletedLinks) {
					metroRailwayMergedLinks.remove(l);
				}
			}while(metroRailwayMergedLinks.size()>0);	// do while not all links have been attempted to be merged
		...
		public static Map<Id<Link>, CustomLinkAttributes> findMostFrequentLinks(int nMostFrequentLinks, boolean extendMetroGrid, boolean entireMetroGrid,
			...
			int i = 0;
			// first just fill map with nMostFrequentLinks links that have traffic > 0
			...
			// add other links from customLinkMap if they have more traffic than current minimum traffic link
			for (Id<Link> linkID : customLinkMap.keySet()) {
			...
			// if this option was chosen in the main class all links accessing a stopFacility from the original scenario
			// are chosen as potential metro stop regions
			if (entireMetroGrid) {
			...
			// this can be specified in the main class in order to make an entire (x-y) grid of potential metro links
			if (extendMetroGrid) {
				Network newNodesNetwork = ScenarioUtils.loadScenario(ConfigUtils.createConfig()).getNetwork();
				System.out.println("It's starting!");
				// find all inks, which definitely have an original stop facility
				// is is important to use only the links with stopFacility access bc later on a dominant stop facility on the link has to be defined
				// if no stop facility is present, the assignment process fails
				List<Id<Link>> linksWithFacility = new ArrayList<Id<Link>>();
				for (TransitStopFacility tsf : ScenarioUtils.loadScenario(ConfigUtils.loadConfig("zurich_1pm/zurich_config.xml")).getTransitSchedule().getFacilities().values()) {
				...
				// make blocks in x-y-grid and search any link with an original stop facility is in a block
				// if one is found, this link can be used as potential metro infrastructure location
				...
				for (int x=-nGridUnits; x<=nGridUnits; x++) {
					for (int y=-nGridUnits; y<=nGridUnits; y++) {
						...
						// check if a link is in the block
...
//							System.out.println("Checking all other facilities...");
							...
							// find from all links in a block the one closest to the center of the block to have the smoothest map coverage
							for (Id<Link> linkId : customLinkMap.keySet()) {
...
//										System.out.println("Found one in block!");
							...
							if (hasMetroLinkInBlock && ! mostFrequentLinks.containsKey(linkInBlock)) {
								// put the closest one to the block center in the map of most frequent links
								mostFrequentLinks.put(linkInBlock, claInBlock);
								// make a node where that link was for visualization
...
//					+ " links (person arrivals + departures) is: " + average);
		...
		public static Map<Id<Link>, CustomLinkAttributes> runPTStopTrafficScanner(
			...
			// add handler that counts traffic on each link
			...
			String eventsFile = "zurich_1pm/Zurich_1pm_SimulationOutputEnriched/ITERS/it." + iterationToRead + "/" + iterationToRead
					+ ".events.xml.gz";
			// count traffic and store results in a custom link map, where every link is marked with its counetd traffic
		...
		// this method scans the previously selected links for all facilities they access.
		// the most dominant facility (rail is strongest of all) is chosen as the location for a potential metro stop
		public static Map<Id<Link>, CustomLinkAttributes> setMainFacilities(TransitSchedule transitSchedule,
			...
			// if user has chosen to use all possible stop facilities, we do not have to check which facility on a link is dominant, we just use any
			if (entireMetroGrid) {
				if (fileName != null) {
		...
		public static Map<Id<Link>, CustomLinkAttributes> findLinksWithinBounds(
			...
			while (linkIterator.hasNext()) {
				...
				distanceFromCenter = GeomDistance.calculate(network.getLinks().get(thisLinkID).getFromNode().getCoord(),
						networkCenterCoord);
				// do not select links that are to close to or too far away from city center
...
//						else {
							...
							for (Entry<Id<Link>, CustomLinkAttributes> customLink : customLinkMap.entrySet()) {
								...
								// - Create and add to network a new node for each domFacility at exactly the Coordinates of domFacility
								// --> Naming: "zhLinkRef"+LinkId"
...
//						}
			...
			// and make sure to also add a customMetroLinkAttribute to each new link in the network,
			// that holds the stopFacilites that it accesses by its nodes
...
//						Log.write("FromNodeStopFacility found and marked: "+thisTSF.getId().toString()+ "  (Link="+newLink.getId().toString()+")");
		...
		public static Network createMetroNetworkFromRailwayNetwork( Map<String, CustomStop> railStops,
			...
			Node newNode;
			for (CustomStop railStop : railStops.values()) {
				// if a railStop is not in inner city and has therefore not yet been added,
				// a new node is created for each railStop and a new metro stopFacility is built thereon
				if (railStop.addedToNewSchedule == true) {
					...
					// keep naming as per the original rail stop naming
			...
			// this entire section is dedicated to making a copy of the railway network and adding it as a part of the metro network
			...
			for (TransitLine tl : tsOriginal.getTransitLines().values()) {
				for (TransitRoute tr : tl.getRoutes().values()) {
					// if a railway transitRoute is found, roam along its stops and always make small metro routes
					// between two consecutive stops along the railway route
					// first connect the last stop to the new route piece, then the links between both stops, and finally connect the current stop along the route
						...
						for (TransitRouteStop RouteStop : tr.getStops().subList(1, tr.getStops().size())) {
							...
							if (railStops.containsKey(lastStopSuperName) == false || railStops.containsKey(currentStopSuperName) == false) {
								...
								if (railStops.get(lastStopSuperName).nextOriginalTransitStopNames.contains(currentStopSuperName) == false) {
									...
									// everytime a new link is built, it has to be added in the metroLinkAttibutesMap with the facilities it touches
...
//			Network nw = sc.getNetwork();
			...
			double distanceFromCenter = 0.0;
			// if a stop facility is not too close to and far enough from city center, it can be used as a terminal
...
//					nw.addNode(nw.getFactory().createNode(Id.createNodeId(tsf.getId().toString()), tsf.getCoord()));
			...
			// do the schedule writing to save a file with all terminal candidates for visualization in VIA and for later use in the code
...
//				networkWriter.write(terminalCandidateNetworkFileName);
		...
		public static ArrayList<NetworkRoute> createInitialRoutesRandom(Network newMetroNetwork, String shortestPathStrategy,
			...
			while (networkRouteArray.size() < nRoutes) {
				// vary the spacing between the end terminals to add diversity in the initial routes formation process
				...
				do {
					...
					// find node below randomly selected terminal stop facility
				...
				do {
					...
					// keep random terminal 2 only if it is within distance constraints of terminal 1
...
//				Log.write("Terminal 2 = "+terminalFacility2.getName() + "   Node="+terminalNode2.toString()+" --> Coord="+newMetroNetwork.getNodes().get(terminalNode2).getCoord());
				...
				// Find Dijkstra shortest path between both terminals --> nodeList
...
//				}
				// convert node list to a linkList and then to a networkRoute
...
//				}
		...
		public static Id<Node> getATerminalNodeId(Network network) {
			List<Id<Node>> idNodesInTerminalRange = new ArrayList<Id<Node>>();
			for (Node node : network.getNodes().values()) {
				if (GeomDistance.calculate(node.getCoord(),new Coord(4000.0, 400.0))<600.0 ||
						GeomDistance.calculate(node.getCoord(),new Coord(4000.0, 7600.0))<600.0 ||
						GeomDistance.calculate(node.getCoord(),new Coord(400.0, 4000.0))<600.0 ||
						GeomDistance.calculate(node.getCoord(),new Coord(7600.0, 4000.0))<600.0){
					idNodesInTerminalRange.add(node.getId());
				}
			}
			return idNodesInTerminalRange.get(new Random().nextInt(idNodesInTerminalRange.size()));
		}


		// this method is used to create exact routes defined by the user
		// the easiest way to do so is to visualize the network and define key the links that the specific routes should visit
...
//	}
	...
	// CAUTION: the name of the reverse link in created by switching the string before and after the "_" of the linkId 
	...
	// this is the entire evolutionary operation block and calls all individual evolutionary operators
	public static MNetworkPop developGeneration(Network globalNetwork, Map<Id<Link>, CustomMetroLinkAttributes> metroLinkAttributes,
			...
			double pMutation, double pBigChange, double pSmallChange, double routeDisutilityLimit,  boolean keepBestRoutes,
...
//		}
		...
		newPopulation = EvoOpsMutator.applyMutations(currentGEN, newPopulation, globalNetwork, zurich_NetworkCenterCoord, lastIterationOriginal,
				pMutation, pBigChange, pSmallChange, routeDisutilityLimit, keepBestRoutes,
...
//			Log.write(mn.networkID+" carries nMetroVehicles="+nVehicles);
	...
	// elite network = network with highest score
	...
	// cross two networks by means of crossing individual routes of the two networks
	// XXX: Please consult thesis for details on procedures
	public static MNetwork[] crossMNetworks(Network globalNetwork, MNetwork parentMNetwork1, MNetwork parentMNetwork2, String vehicleTypeName, double vehicleLength, double maxVelocity, 
		...
		// make clones of parents to directly modify candidates and place them in child networks
		...
		Loop1:
		while (iter1.hasNext()) {
			// try all routes of parent 1 against all routes of parent 2
			// when finding an intersection between the routes, cross them straight away
	...
	public static MRoute[] crossMRoutes(MRoute routeFromP1, MRoute routeFromP2, Network globalNetwork,
		...
		// four cutting and merging strategies may be pursued
		// two options for the direction of both routes with which they are cut
		// note that the linkList of the MRoute is two ways (there and back) and therefore only one sequence (there) is required to find intersections
		...
		while(routesCandidates1.size() > 0 && crossingFound == false) {
			...
			Random r = new Random();
			ri = r.nextInt(4);
			// pick a random crossing strategy
			}while(routesCandidates1.keySet().contains(ri)==false);
			// remove strategy if it was attempted so that if it fails, the next one can be tried
			...
			// check every node of route 1 against nodes of route 2 to find intersections
			for (Id<Link> linkFrom1 : route1LinkListOneway.subList((int) Math.floor((minCrossingDistanceFactorFromRouteEnd)*routeLength1), (int) Math.ceil((1-minCrossingDistanceFactorFromRouteEnd)*routeLength1))) {
				...
				for (Id<Link> linkFrom2 : route2LinkListOneway.subList((int) Math.floor((minCrossingDistanceFactorFromRouteEnd)*routeLength2), (int) Math.ceil((1-minCrossingDistanceFactorFromRouteEnd)*routeLength2))) {
					...
					if (fromNode1.getId().toString().equals(fromNode2.getId().toString())) {
						...
						// check links before and after the intersection node to check if the routes could
						// be merged together after cutting both into two pieces
						...
						// only if crossing angles of new merge are OK, the crossing takes place
		...
		if(crossingFound) {
			...
			// reattach routes in different composition
			...
			// build the corresponding MRoutes
			...
			// the MRoute attributes of the new routes are generated by the average of the parents
	...
	// Consult thesis and literature referred to therein for details on roulette wheel implementations
...
//		Log.write("networkScoreMapPositiveBenefits = "+networkScoreMapPositiveBenefits.toString());
		...
		// Option 1 : All positive overallScoreProportional = selection probability is proportional to its welfare score
		...
		// Option 2 : overallScoreRank = selection probability is proportional to its rank among all networks and their scores
		...
		// Option 3 : Tournament selection = choosing three random networks and selecting strongest of the three as the final choice
		...
		// Option 4 : logarithmic = selection proportional to logarithmic scores of networks
	...
	// just load and return globalNetwork
	public static Network updateGlobalNetwork() {
		Config config = ConfigUtils.createConfig();
	...
	public static boolean logResults(List<Map<String, NetworkScoreLog>> networkScoreMaps, String historyFileLocation,
		...
		for (String networkName : latestPopulation.getNetworks().keySet()) {
			...
			if(latestPopulation.modifiedNetworksInLastEvolution.contains(mnetwork.getNetworkID())) {
				...
				mnetwork.lifeTime = lifeTime;
				// XXX: This is the essential step: within this method the network score=welfare is calculated
				...
				// performance goals are deprecated and their code must not be considered here
			...
			// put scores of network in a map of all networks that is stored for every generation
			// so that later the evolution of the scores can be recalled and visualized
		...
		// add the map of all network scores for this generation
	...
	public static Map<String,CustomStop> getOriginalRailwayStations(double radius, Coord zurich_NetworkCenterCoord,
		...
		// go through original schedule to find all rail lines
		for (TransitLine tl : ts.getTransitLines().values()) {
			for (TransitRoute tr : tl.getRoutes().values()) {
				if (tr.getTransportMode().toString().equals("rail")) {
					// PART A: Isolate rails network for visualization
					// take route, convert it to a link list and build a new (visualization) network from the links
					List<Id<Link>> linkList = NetworkRoute2LinkIdList(tr.getRoute());
					for (Id<Link> linkId : linkList) {
					...
					for (TransitRouteStop trs : tr.getStops()) {
						...
						// stopFacility has format id="8500562.link:920757".
						// First part (number) is unique to the stop, but it can have several refLinks, which are the second part
						if (railStopsZH.keySet().contains(stopName) == false) {
							// add all original rail stops to the map
	...
	// remove several substrings from a string
...
//		XMLOps.readFromFile(railStop.getClass(), "zurich_1pm/Evolution/Population/BaseInfrastructure/metroStopAttributes.xml"));
	...
	// clean certain generations of the history log to free up some space
	// keep one generation once every pre-specified interval
